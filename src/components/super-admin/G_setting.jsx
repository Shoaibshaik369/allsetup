  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update the latestId in localStorage if it's a new record
        if (!isUpdate) {
          localStorage.setItem('latestId', result.id || (parseInt(localStorage.getItem('latestId') || '0') + 1));
        }
